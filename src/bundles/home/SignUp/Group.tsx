...
class Group extends React.Component<{}, {}> {
		...
		return (
			<section className='page small'>
				...
				<form className='group' onSubmit={this._onSubmit}>
					...
					<Input title='Group name' block={true} onChange={this._onGroupNameChange} props={{ type: 'text' }} ref='first' />
	...
	private _onGroupNameChange = (value: string) => {
	...
	private _onSubmit = (event: React.FormEvent<HTMLFormElement>) => {
		...
		if (this._groupName && this._email && this._password && this._confirmPassword) {
			API.signUp(true, this._groupName, this._email, this._password, this._confirmPassword)
				.then(() => {
